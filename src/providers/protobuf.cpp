...
struct Convert
{
    ...
    static void decode(Value<ValType>& node, const ConstWalkType& proto)
    {
        ...
        if constexpr (ValType == Type::Bool) {
            ...
            node = UString(refl->GetString(*std::get<0>(proto), std::get<1>(proto)));
        ...
        } else if constexpr (ValType == Type::Bytes) {
            static_cast<Binary&>(node).setString(refl->GetString(*std::get<0>(proto), std::get<1>(proto)));
        }
    }
    ...
    static void decode(List<ListType>& node, const ConstWalkType& proto)
    {
        ...
        for (int i = 0; i < refl->FieldSize(*std::get<0>(proto), std::get<1>(proto)); ++i) {
            if constexpr (ValType == Type::Bool) {
                ...
                node.append(UString(refl->GetRepeatedString(*std::get<0>(proto), std::get<1>(proto), i)));
            } else if constexpr (ValType == Type::UInt32) {
                ...
                node.append(Binary::fromString(refl->GetRepeatedString(*std::get<0>(proto), std::get<1>(proto), i)));
            }
        }
    }
    ...
    static void encode(const Value<ValType>& node, WalkType& proto)
    {
        ...
        if constexpr (ValType == Type::Bool) {
            ...
            refl->SetString(std::get<0>(proto), std::get<1>(proto), node.value().toStdString());
        } else if constexpr (ValType == Type::UInt32) {
            refl->SetUInt32(std::get<0>(proto), std::get<1>(proto), node.value());
        } else if constexpr (ValType == Type::UInt64) {
            refl->SetUInt64(std::get<0>(proto), std::get<1>(proto), node.value());
        } else if constexpr (ValType == Type::Bytes) {
            refl->SetString(std::get<0>(proto), std::get<1>(proto), static_cast<const Binary&>(node).asString().toStdString());
        }
    }
    ...
    static void encode(const List<ListType>& node, WalkType& proto)
    {
        ...
        auto setFunc = [&](const auto& val) {
            if constexpr (ValType == Type::Bool) {
                ...
                refl->AddString(std::get<0>(proto), std::get<1>(proto), val.value().toStdString());
            } else if constexpr (ValType == Type::UInt32) {
                refl->AddUInt32(std::get<0>(proto), std::get<1>(proto), val);
            } else if constexpr (ValType == Type::UInt64) {
                refl->AddUInt64(std::get<0>(proto), std::get<1>(proto), val);
            } else if constexpr (ValType == Type::Bytes) {
                refl->AddString(std::get<0>(proto), std::get<1>(proto), static_cast<const Binary&>(val).asString().toStdString());
            }
        };
...
public:
        ...
        if (val.isValueList()) {
            switch (val.valueType()) {
                ...
                case Type::String:
                    ...
                    break;
                case Type::Bytes:
                    Convert<Type::Bytes>::encode(static_cast<const BinaryList&>(val), proto);
                    break;
                case Type::Unknown:
                    break;
            }
    ...
    static void packValue(const Node& node, WalkType& proto, Option opt)
    {
        for (auto& it : node.meta().fields()) {
            if (!it->empty()) {
                auto fdesc = std::get<0>(proto)->GetDescriptor()->FindFieldByName(it->key().toStdString());
                if (fdesc && fdesc->cpp_type() == pb::FieldDescriptor::CPPTYPE_MESSAGE && !fdesc->is_repeated()) {
                    ...
                    throw std::runtime_error(std::format("Cannot find {}", it->key()));
                }
            }
        }
    }
...
public:
        ...
        if (list.isValueList()) {
            switch (list.valueType()) {
                ...
                case Type::String:
                    ...
                    break;
                case Type::Bytes:
                    Convert<Type::Bytes>::decode(static_cast<BinaryList&>(list), proto);
                    break;
                case Type::Unknown:
                    break;
            }
    ...
    static void unpackValue(Node& node, const WalkType& proto)
    {
        for (auto& it : node.meta().fields()) {
            auto fdesc = std::get<0>(proto)->GetDescriptor()->FindFieldByName(it->key().toStdString());
            if (fdesc && fdesc->cpp_type() == pb::FieldDescriptor::CPPTYPE_MESSAGE && !fdesc->is_repeated()) {
